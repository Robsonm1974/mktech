...
function CriarAulaForm() {
  ...
  const anoDetectado = blocoReferencia?.ano_escolar_id 
    ...
    : null
  const disciplinaDetectada = blocoReferencia?.disciplinas && disciplinas.length > 0
    ? (() => {
        if (Array.isArray(blocoReferencia.disciplinas)) {
          const firstDisc = blocoReferencia.disciplinas[0]
          return firstDisc ? disciplinas.find(d => d.codigo === firstDisc.codigo) : null
        }
        const disc = blocoReferencia.disciplinas as { codigo?: string }
        return disc.codigo ? disciplinas.find(d => d.codigo === disc.codigo) : null
      })()
    : null
